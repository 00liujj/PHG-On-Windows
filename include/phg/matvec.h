...
/* $Id: matvec.h,v 1.1 2014/04/04 04:30:01 zlb Exp $ */
...
PHG_EXTERN void phgMatSetupDiagonal(MAT *mat);
PHG_EXTERN void phgMatPack(MAT *mat);
PHG_EXTERN void phgMatUnpack(MAT *mat);

PHG_EXTERN MAT *phgMatAXPBY_(FLOAT a, MAT *x, FLOAT b, MAT **py,
...
PHG_EXTERN MAT *phgMatGetOffprocRows(MAT *B, INT n, const INT list[],
			  BOOLEAN list_is_ordered, const INT ordering[]);
PHG_EXTERN MAT *phgMatTranspose(MAT *A);
PHG_EXTERN MAT *phgMatMat(MAT_OP transa, MAT_OP transb, FLOAT alpha, MAT *A, MAT *B,
		FLOAT beta, MAT **Cptr);

PHG_EXTERN VEC *phgVecCreate(MPI_Comm, INT m, INT M, int nvec);
PHG_EXTERN void phgVecDestroy(VEC **vec);
PHG_EXTERN VEC *phgVecCopy(VEC *src, VEC **dest);
PHG_EXTERN VEC *phgVecAXPBY(FLOAT a, VEC *x, FLOAT b, VEC **y);

PHG_EXTERN VEC *phgMatVec(MAT_OP op, FLOAT alpha, MAT *A, VEC *x, FLOAT beta, VEC **y);
#define phgMatMultVec(mat, vec, result) \
	 phgMatVec(MAT_OP_N, (FLOAT)1.0, mat, vec, (FLOAT)0.0, result)
    
PHG_EXTERN FLOAT phgVecDot(VEC *x, int which_x, VEC *y, int which_y, FLOAT *res);
PHG_EXTERN FLOAT phgVecNorm2(VEC *src, int which, FLOAT *res);
PHG_EXTERN FLOAT phgVecNorm1(VEC *src, int which, FLOAT *res);
PHG_EXTERN FLOAT phgVecNormInfty(VEC *src, int which, FLOAT *res);

PHG_EXTERN MAT *phgMatDup(MAT *src);
PHG_EXTERN void phgVecDumpMATLAB(VEC *v, const char *var_name, const char *file_name);
PHG_EXTERN void phgMatDumpMATLAB(MAT *A, const char *var_name, const char *file_name);
PHG_EXTERN VEC *phgVecRandomize(VEC *v, long int seed);

PHG_EXTERN MAT *phgMatCreate(MPI_Comm comm, INT m, INT M);
PHG_EXTERN MAT *phgMatCreateNonSquare(MPI_Comm comm, INT m, INT M, INT n, INT N);
PHG_EXTERN MAT *phgMatCreateMatrixFree(MPI_Comm comm, INT m, INT M,
				MV_FUNC mv_func, void *mv_data, ...);
PHG_EXTERN MAT *phgMatCreateMatrixFreeNonSquare(MPI_Comm comm, INT m, INT M, INT n, INT N,
				MV_FUNC mv_func, void *mv_data, ...);
PHG_EXTERN MAT *phgMatCreateBlockMatrix(MPI_Comm comm, int p, int q, MAT *pmat[],
			     FLOAT coeff[], MAT_OP trans[]);
PHG_EXTERN const MAT_ROW *phgMatGetRow_(MAT *mat, INT irow, MAT_ROW *row0);
#define phgMatGetRow(mat, irow) phgMatGetRow_(mat, irow, NULL)
PHG_EXTERN INT phgMatGetNnzInRow(MAT *mat, INT irow, INT *nd, INT *no);
PHG_EXTERN void phgMatFreeMatrix(MAT *mat);
PHG_EXTERN void phgMatDestroy(MAT **Mat);

PHG_EXTERN struct MAP_ *phgVecGetMap(VEC *vec);
PHG_EXTERN struct MAP_ *phgMatGetRowMap(MAT *mat);
PHG_EXTERN struct MAP_ *phgMatGetColumnMap(MAT *mat);

PHG_EXTERN void phgMatAssemble(MAT *mat);
PHG_EXTERN void phgMatDisassemble(MAT *mat);
PHG_EXTERN void phgVecAssemble(VEC *vec);
PHG_EXTERN void phgVecDisassemble(VEC *vec);

PHG_EXTERN void phgMatSetMode(MAT *mat, MAT_MODE mode);
#define phgMatSetReplaceMode(mat)	phgMatSetMode(mat, PHG_REPLACE)
#define phgMatSetAddMode(mat)		phgMatSetMode(mat, PHG_ADD)
PHG_EXTERN void phgMatAddEntry_(MAT *mat, INT row, INT col, FLOAT value,
...
PHG_EXTERN void phgMatAddEntries_(MAT *mat, INT nrows, INT *rows, INT ncols, INT *cols,
			FLOAT *values, BOOLEAN rows_are_global,
...
PHG_EXTERN void phgVecAddEntries_(VEC *vec, int which, INT n, INT *indices, FLOAT *values,
		       BOOLEAN global);
PHG_EXTERN void phgVecAddEntry(VEC *vec, int which, INT index, FLOAT value);
PHG_EXTERN void phgVecAddGlobalEntry(VEC *vec, int which, INT index, FLOAT value);
...
PHG_EXTERN MAT *phgMatRemoveBoundaryEntries(MAT *mat);

PHG_EXTERN int phgMatEigenSolve(MAT *A, MAT *B, int n, int which, FLOAT tau, int *nit,
		     FLOAT *evals, struct MAP_ *map, DOF **u, ...);

PHG_EXTERN int phgMatDumpCSR(MAT *mat, const char *fn);
PHG_EXTERN int phgVecDump(VEC *vec, const char *fn);
