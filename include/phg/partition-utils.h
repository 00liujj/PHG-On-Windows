...
/* $Id: partition-utils.h,v 1.1 2014/04/04 04:30:01 zlb Exp $ */
...
/* used by phgImport                      */
/* Hilbert order */
PHG_EXTERN int phgHilbertOrder(GRID *g);
/* Morton order */
PHG_EXTERN int phgMortonOrder(GRID *g);

/* initial order */
/* 0: success, or fail */
PHG_EXTERN int phgGridInitOrder(GRID *g, BOOLEAN dist);
...
/*            version 1                         */
PHG_EXTERN FLOAT phgPartition1DV1(DOTS *x, INT lenx, int np, const FLOAT speeds[],
			MPI_Comm comm);
PHG_EXTERN FLOAT phgPartition1DV2(DOTS *x, INT lenx, int np, const FLOAT speeds[],
			MPI_Comm comm);

/* internal function used to register parameters */
PHG_EXTERN void phgPartition1DOptionsSet();
...
PHG_EXTERN int phgPartitionRemap(GRID *g, MPI_Comm newcomm);
PHG_EXTERN int phgPartitionRemap0(MPI_Comm comm, const double datasize[], int perm[],
		       MPI_Comm newcomm);

/* print quality information */
PHG_EXTERN void phgPartitionQuality(GRID *g);

/* Partitioner user */
PHG_EXTERN BOOLEAN phgPartitionerUser(GRID *g, MPI_Comm newcomm, DOF *weights, FLOAT power);
#endif
