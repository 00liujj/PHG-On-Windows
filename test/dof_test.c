 ...
 * $Id: dof_test.c,v 1.91 2014/06/08 02:11:59 zlb Exp $ */
...
/*----------------------------------------------------------------------------*/
	...
	if (j < DofGetDataCount(u))
	    break;
	tmp->data[i] = 0.0;
    }
    if (i >= DofGetDataCount(u))
	phgPrintf("\npassed\n");
skip:
    phgDofFree(&u);
...
skip1:
    ...
    for (i = 0; i < post_refines
			&& DofGetDataCountGlobal(u) / g->nprocs < 500000; i++) {
