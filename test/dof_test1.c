 ...
 * $Id: dof_test1.c,v 1.63 2012/09/13 08:12:31 zlb Exp $ */
...
#if USE_MPI
      a = err_L2;
      MPI_Allreduce(&a, &b, 1, PHG_MPI_FLOAT, MPI_SUM, g->comm);
      err_L2 = Sqrt(b); 
      /*
      a=err_infinity;
      MPI_Allreduce(&a, &b, 1, PHG_MPI_FLOAT, MPI_MAX, g->comm);
      err_infinity = Sqrt(b);
      */
#else 
      err_L2 = Sqrt(err_L2);
...
while(TRUE)
{
      ...
      phgPrintf ("%d DOF, %d elements, load imbalance: %lg\n",
			DofGetDataCountGlobal(u), g->nleaf_global,
			(double)g->lif);
	...
	check_basfunc(g,u,e);
      ...
      err_L2=compute_HCurlerror(u,QUAD_3D_P1,f,curl_f);
