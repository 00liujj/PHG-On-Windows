...
/*******************************************************
* $Id: non-smooth.c,v 1.62 2012/11/20 08:35:55 zlb Exp $
*
...
SolveEqua(OEM_SOLVER *sol, DOF *u_h, DOF *f_h, DOF *ax)
{
    ...
    SOLVER *solver;
    int nits;
    phgPrintf("DOF: %"dFMT" ", DofGetDataCountGlobal(u_h));
...
#if USE_MPI
        MPI_Allreduce(&tmp, &result, 1, PHG_MPI_FLOAT, MPI_SUM, u1->g->comm);
#else
        result = tmp;
#endif
...
#if USE_MPI
    MPI_Allreduce(&sum, &tmp, 1, PHG_MPI_FLOAT, MPI_SUM, g->comm);
    sum = tmp;
#endif
