 ...
 *
 *   where
 *   	Fp <==>	I + dt (-1/Re \laplace + u \cdot \grad)
 ...
 * 
 * $Id: navier-stokes-unsteady.c,v 1.12 2015/04/07 07:23:59 wleng Exp $ */
...
//static FLOAT Theta = 1.;              /* Backward Eluer */
static FLOAT Theta = .5;	/* Crank-Nicolson */
...
main(int argc, char *argv[])
{
    ...
    DOF *u, *gradu, *p, *u_old, *gradu_old, *p_old, *u0, *p0,
	*f, *pbc, *gxbc, *gybc, *gzbc, *vortex,
	*dofs[64], *u_exact, *p_exact, *gradu_exact, *eu, *ep,
	*egradu, *divu = NULL, *u1, *du;
    SOLVER *solver, *pc = NULL;
    MAP *Vmap, *V1map, *Pmap, *Pmap0;
    MAT *pmat[4], *mats[16];
    FLOAT time_end = 1e2, res, tol = 1e-3, max_nontol = 1e-8;
    INT tstep = 0, max_tstep = 1e8, nonstep = 0, max_nonstep = 50;
    ...
    phgOptionsRegisterFloat("-max_non_tol", "Max nonlinear iter tolerance",
			    &max_nontol);
    phgOptionsRegisterFloat("-theta", "time fraction coef", &Theta);
    phgOptionsRegisterInt("-mem_max", "Max memory per process", &mem_max);
    phgOptionsRegisterInt("-pre_refines", "Pre-refines", &pre_refines);
    phgOptionsRegisterInt("-max_time_step", "Max time step", &max_tstep);
    phgOptionsRegisterInt("-max_nonstep", "Max newton iter step",
			  &max_nonstep);
...
#if 0
    /* refine mesh to 20K elements / process */
    phgRefineAllElements(g, 9);
    while (g->nleaf_global / g->nprocs < 20000) {
	ELEMENT *e;
	int i;
	phgBalanceGrid(g, 1.2, 1, NULL, 0.);
	ForAllElements(g, e) {
	    for (i = 0; i < NFace; i++)
		if (g->types_face[e->faces[i]] & BDRY_MASK)
		    break;
	    e->mark = (i < NFace ? 1 : 0);
	}
	phgRefineMarkedElements(g);
	phgPrintf("Pre-refinement: %"dFMT" elements\n", g->nleaf_global);
    }
#else
    ...
    u1 = phgDofNew(g, utype, 1, "u0", DofInterpolation);
...
#warning dt
...
#if OUTPUT_VTK
    if (TRUE) {
	phgExportVTK(g, "ins_init.vtk", u, p, NULL);
	phgExportDX(g, "ins_init.dx", u, p, NULL);
    }
#endif /* OUTPUT_VTK */

    /* Time advancing. */
    tstep = 0;
    while (TRUE) {		/* Time advacing */
	FLOAT last_res;
	...
	phgPrintf("\n==========\ntime:%lf, step:%"dFMT"\n",
			(double)time, tstep);
	phgPrintf("%"dFMT" DOF, %"dFMT" elements, %d submesh%s, "
		  "load imbalance: %lg\n",
	...
	/* update non-linear unkonws as time advancing */
	phgDofFree(&u_old);
	phgDofFree(&p_old);
	phgDofFree(&gradu_old);

	u_old = u;
	p_old = p;
	gradu_old = gradu;

	u = phgDofCopy(u_old, NULL, utype, "u");
	p = phgDofCopy(p_old, NULL, ptype, "p");
	p->DB_mask = 0;
	gradu = phgDofGradient(u, NULL, NULL, "grad u[3]");
	...
	while (TRUE) {
	    ...
	    phgPrintf("Newton step %2"dFMT"\n", nonstep);

	    /* update non-linear unkonws */
	    phgDofFree(&u0);
	    phgDofFree(&p0);
	    u0 = u;
	    p0 = p;
	    u = phgDofCopy(u0, NULL, utype, "u");
	    p = phgDofCopy(p0, NULL, ptype, "p");
	    p->DB_mask = 0;

	    Vmap = phgMapCreate(u, NULL);
	    V1map = phgMapCreate(u1, NULL);
	    ...
	    packDof(dofs, 9, u0, p0, gradu, divu, f, pbc, gxbc, gybc, gzbc);	/* gradu = gradu0 */
	    build_matrices(solver, pc, dofs, mats, PICARD);

	    /* Note: if picard(fixed point) iter is usesd,
	     * then there is no need to update rhs every nonlinear step. */
	    packDof(dofs, 9, u_old, p_old, gradu_old, divu, f, pbc, gxbc,
		    gybc, gzbc);
	    ...
	    assert(gradu != NULL);
	    phgDofGradient(u, &gradu, NULL, "grad u[3]");
	    //      last_res *= 1e-2;//solver->residual;
	    last_res = 1e-1 * solver->residual;
	    ...
	    /* correction of u in non linear iteration */
	    du = phgDofCopy(u0, NULL, NULL, NULL);
	    phgDofAXPY(-1.0, u, &du);
	    res = phgDofNormL2(du);
	    phgDofFree(&du);
	    phgPrintf("\tnon_correction_L2 = %le\n", (double)res);
	    if (nonstep++ >= max_nonstep || res < max_nontol)
		break;
	...
	/* add divu check in future */
	phgPrintf("L2 = (%0.6le,%0.6le)\n            H1 = (%0.6le), "
		  , phgDofNormL2(eu), phgDofNormL2(ep),
		  res = phgDofNormL2(egradu));
	elapsed_time(g, TRUE, 0.);
