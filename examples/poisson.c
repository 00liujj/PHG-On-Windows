...
/* $Id: poisson.c,v 1.328 2013/11/26 09:35:28 zlb Exp $ */
...
#if 0
/* Note: the following code is only used to test the solver interface for
 *	 the case u_h->dim > 1 */
static void
build_linear_system(SOLVER *solver, DOF *u_h, DOF *f_h)
{
    ...
    ELEMENT *e;
    int i, j, k;
    FLOAT A[N][N], B[N][dim], buffer[N], *row;
    INT I[N][dim], J[dim][N];
...
build_linear_system(SOLVER *solver, DOF *u_h, DOF *f_h)
{
    ...
    ForAllElements(g, e) {
	int N = DofGetNBas(u_h, e);    /* # basis functions in the element */
...
/* test solving multiple RHS with a same matrix */
{
    ...
    FLOAT *data = phgAlloc(DofGetDataCount(u_h) * sizeof(*u_h->data));
    ...
    memcpy(data, u_h->data, DofGetDataCount(u_h) * sizeof(*u_h->data));
    ...
    memcpy(u_h->data, data, DofGetDataCount(u_h) * sizeof(*u_h->data));
    phgFree(rhs);
    phgFree(rhs_offp);
    phgFree(data);
}
	...
	phgPrintf("Assemble stiffness matrix: ");
	...
	phgPrintf("Memory usage: current %0.4lg/%0.4lg/%0.4lgMB, "
	    ...
	    const char *p;
	    if (export_all)
		sprintf(fn, "mesh%03d.%s", g->serial_no % 65536,
